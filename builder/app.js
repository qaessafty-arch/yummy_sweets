...
const app = {
  ...
  handleCustomerSubmit(e) {
    ...
    if (phone.replace(/\D/g, '').length < 6) {
      this.showToast(this.t('toastInvalidPhone'), 'error');
      return;
    }

    // Auto-create or refresh customer record
    let existing = this.users.find(u => u.phone === phone);
    if (!existing) {
      existing = {
        id: 'c_' + Date.now(),
        phone: phone,
  ...
  handleStaffSubmit(e) {
    ...
    if (!staff) {
      this.showToast(this.t('toastInvalidLogin'), 'error');
      return;
    }
    ...
    this.renderWatermark();
    this.closeAuthModal();
    this.showToast(this.t('loggedInAs').replace('{role}', staff.role.toUpperCase()), 'success');

    // Auto-open control panel after ~400ms
    setTimeout(() => {
      this.openPanelModal();
    }, 400);
  },
